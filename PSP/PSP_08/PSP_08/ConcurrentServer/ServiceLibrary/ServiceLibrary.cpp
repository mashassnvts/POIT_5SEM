...
// это экспортируемая функция из DLL, которая используется для запуска конкретного сервиса, указанного по его идентификатору 
// Функция SSS - точка входа для запуска сервиса по его идентификатору и передаче параметров
