...
DWORD WINAPI DispatchServer(LPVOID pPrm);//Диспетчеризация запросов клиента
DWORD WINAPI ResponseServer(LPVOID pPrm);//широковещательные запросы от клиентов

CRITICAL_SECTION scListContact;

enum TalkersCommand //команды управления
{
...
void CommandsCycle(TalkersCommand& cmd)//clientNp
{
    int  sq = 0;//sostoyanie
    while (cmd != EXIT)
    {
...
TalkersCommand set_param(char* param)//clientNP
{
...
DWORD WINAPI DispatchServer(LPVOID pPrm)//clientT
{
    DWORD rc = 0;
            ...
            for (auto client = contacts.begin(); client != contacts.end(); client++)
            {
                if (client->type == client->ACCEPT)
                {
                    ...
                    else
                    {
                        ...
                        CancelWaitableTimer(client->htimer); // Останавливаем текущий таймер
...
DWORD WINAPI GarbageCleaner(LPVOID pPrm)//clientB
{
    DWORD rc = 0;
...
DWORD WINAPI ConsolePipe(LPVOID pPrm)//cleintNP
{
    DWORD rc = 0;
    char rbuf[100];
...
// получить запрос от клиента, проверить его имя (или позывной), и если оно совпадает с ожидаемым, сохранить адрес клиента.
bool GetRequestFromClient(char* name, short port, SOCKADDR_IN* from, int* flen)//clientNP
{
    SOCKADDR_IN clnt;
    int lc = sizeof(clnt);
    ZeroMemory(&clnt, lc);
